...
### ⛔ push は毎回、その場で許可を取る（最重要）

**`git push` = 即本番デプロイ。** このリポジトリに検証環境としての段階は無く、
push した瞬間に taitech.dev が書き換わる。したがって:

**push の直前に、そのコミットについて許可を取る。許可が無ければ commit までで止めて、
差分を提示して待つ。** 「実装して」と言われても、それは push の許可ではない。

**以下はいずれも許可ではない。** すべて 2026-08-29 に実際に取り違えた形。

| 取り違えやすいもの | なぜ許可ではないか |
|---|---|
| **質問への回答をもらった** | 事実を教えてもらっただけ。**これで push したのが 2026-08-29 の `b971f96`** |
| 「実装して」「進めて」「やって」 | 実装の許可であって公開の許可ではない |
| 前のコミットで push の許可が出た | **許可はコミット 1 個ぶんで切れる。** 連続作業でも毎回取り直す |
| こちらが「デプロイにまとめられます」と言い、相手が「進めましょう」と答えた | **提案の範囲が変わったら取り直す。** 3 件まとめると言って 2 件で push するのは別物 |
| ローカルの検証が全部通った | 品質の話であって、公開してよいかとは無関係 |

**迷ったら push しない。** commit を積んで「push しますか」と聞くコストはほぼゼロで、
逆に意図しない本番反映は取り消せない（キャッシュ・クローラ・SNS のプレビューに残る）。

**docs/ の変更だけなら push しない**（`.gitignore` 済みなのでそもそも push 対象外）。
...
| パス | 中身 |
|---|---|
| `docs/strategy/` | `status.md` (**現状・KPI・ブロッカー。まずここ**) / `kpi.md` (KGI の分解と月次目標) / `roadmap.md` (未完了タスク) / `growth.md` (認知獲得) / `seo.md` / `ideas.md` |
...
- **テストでタグ付きリンクを踏まない**。デプロイ後のリンク確認は **`?tag=taitech-22` を外した URL** で開く。Amazon 側のクリック数には GA4 のような内部トラフィックフィルタが**無い**ので、自己クリックがそのまま計上され、**KPI の分母が壊れる**。実測で Amazon 側 62 件に対し GA4 側 8 件（7〜8 倍）まで膨れており、日別の山が**デプロイ日と一致**していた（`docs/strategy/kpi.md` §2-1）。**遡って除去する手段は無い**ので、汚さないことでしか守れない。規約面でも、自分でリンクを踏んだ 24 時間以内の買い物が自己購入扱いになるリスクを避けられる
